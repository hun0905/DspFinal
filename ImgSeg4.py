...
def getFood(img):
    #將影像進行濾波並找出其輪廓
    im_size = img.size/(3*512*512)
    hsv_img = cv2.cvtColor(img ,cv2.COLOR_BGR2HSV)
    if np.mean(hsv_img[:,:,2])<60:
        alpha=3#1.5
        beta=50
        img = cv2.convertScaleAbs(img, alpha=alpha, beta=beta)
    # img_filt=cv2.GaussianBlur(img, (15, 15), 0)
    ...
    img_th = cv2.adaptiveThreshold(img_filt,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,5,2)
    ...
    while cv2.contourArea(contours[i]) > 200000*im_size or NotPlate:
        mask2 = mask.copy()
        ...
        if mask2[int(center[0]),int(center[1])]== 255 and cv2.contourArea(contours[i]) < 200000:
            ...
            i-=1
            NotPlate=True
        if cv2.contourArea(contours[i]) < 100000*im_size:
            NotPlate=False
    if cv2.contourArea(contours[i]) < 100000*im_size:
    ...
    img_bin=cv2.cvtColor(food_area, cv2.COLOR_BGR2GRAY)

    #以contour的面積來找，找到大小合理的contour，並判定這些contour是食物的cnotour，將contour外的影像都去除，指保留conotour內的組成，並存到food

    _, img_th = cv2.threshold(food_area, 105, 255, 0)   
    # img_filt=cv2.cvtColor( food_area , cv2.COLOR_BGR2GRAY)
    # img_th = cv2.adaptiveThreshold(img_filt,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,5,2)

    img_th = cv2.Canny(img_th, 120, 200)
    
    kernelClose=np.ones((8,8))
    img_th = cv2.morphologyEx(img_th, cv2.MORPH_CLOSE, kernelClose)
    ...
    try:
        i=-1
        while cv2.contourArea(contours[i]) > 100000*im_size:
            i-=1
        while i>=-len(contours):
            ...
            for cnt in added_food_contour:
                ...
                if np.sqrt(((x1+w1)/2-(x2+w2)/2)**2+((y1+h1)/2-(y2+h2)/2)**2)<50:
                    NotAdded = False
            if cv2.contourArea(contours[i]) >1000*im_size and NotAdded:
                img_tmp = img.copy()
                mask_food = np.zeros(img_bin.shape, np.uint8)
                mask_tmp = np.ones(img.shape, np.uint8)*255
                # cv2.drawContours(mask_food, [contours[i]], 0, (255,255,255), -1)
                cv2.drawContours(mask_food, [contours[i]], 0, (255,255,255), -1)
                cv2.drawContours(mask_tmp, [contours[i]], 0, (0,0,0), -1)
                food_area.append(cv2.contourArea(contours[i]))
                kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(13,13))
                mask_food = cv2.dilate(mask_food,kernel,iterations = 1)
                # print(cv2.bitwise_or(cv2.bitwise_and(img,mask = mask_food),mask_tmp))
            
                tmp=cv2.add(cv2.bitwise_and(img_tmp,img,mask = mask_food),mask_tmp)
            ...
            elif cv2.contourArea(contours[i]) <1000*im_size:
                break
            i-=1
    except:
    ...
    Pixel2cm = 1/10
    try:
        while cv2.contourArea(contours[i]) > 10000*im_size:
            i-=1
