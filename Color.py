...
def getColorFeature2(img):
	
    img_hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    h,s,v = cv2.split(img_hsv)
    # hsvHist = [[[0 for _ in range(4)] for _ in range(4)] for _ in range(8)]
    features = []
    hist = cv2.calcHist([img_hsv], [0, 1, 2], None, [60,1,1], [0, 180, 0, 256, 0, 256]).flatten()
    # hist2 = cv2.calcHist([img], [0, 1, 2], None, [2,2,2], [0, 256, 0, 256, 0, 256]).flatten()[1:]
    # feature = np.concatenate((hist,hist2),axis=0)
    top3=hist.flatten()[1:]
    feature =top3.argsort()[-1:-6:-1]
